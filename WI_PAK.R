...
main <- readRDS("C:/Users/LEOPOLD/OneDrive - UNHCR/Work/DHS Wealth index/main_anon_pakistan.rds")
...
table(main$Land01, main$Intro_07)# -- dont use land ownership
table(main$House01, main$Intro_07)# --  use house ownership
...
#Dwelling materials: 
##In Pakistan, if HH01 housing type 4 (tent) or 5 (caravan), no building materials specified
###all obs HH01 = 4 (tent) we assume walls =no walls, roof=no roof, floor = earth/sand
###(HH01=5 only 3 observations)
main <- main %>%
  mutate(
    HH02 = ifelse(HH01 == 4 & is.na(HH02), 11, HH02),
    HH03 = ifelse(HH01 == 4 & is.na(HH03), 11, HH03),
    HH06 = ifelse(HH01 == 4 & is.na(HH06), 11, HH06)
  )
...
main <- main %>%
  ...
  mutate(
    ...
    Assets04h_1 = ifelse(Assets04h >= 1 & Assets04h <= 4, 1, 0),
    Assets04h_2 = ifelse(Assets04h >= 5 & Assets04h <= 9, 1, 0),
    Assets04h_3 = ifelse(Assets04h >= 10 & Assets04h != 99, 1, 0)
  )%>%
  select(-c(Assets04a, Assets04b, Assets04c, Assets04d, Assets04e, Assets04f, Assets04h, Assets03))
...
zero_sd_vars_main <- sapply(main_WI[ , 8:ncol(main_WI)], function(x) sd(x, na.rm = TRUE) == 0)
removed_vars_main <- names(main_WI)[which(zero_sd_vars_main) + 7]  # +6 to account for starting at column 5
main_WI <- main_WI[ , !(names(main_WI) %in% removed_vars_main)]
cat("Removed variables from main:", removed_vars_main, "\n")

zero_sd_vars_urban <- sapply(main_WI_urban[ , 8:ncol(main_WI_urban)], function(x) sd(x, na.rm = TRUE) == 0)
removed_vars_urban <- names(main_WI_urban)[which(zero_sd_vars_urban) + 7]  # +6 to account for starting at column 5
main_WI_urban <- main_WI_urban[ , !(names(main_WI_urban) %in% removed_vars_urban)]
cat("Removed variables from urban:", removed_vars_urban, "\n")


zero_sd_vars_rural <- sapply(main_WI_rural[ , 8:ncol(main_WI_rural)], function(x) sd(x, na.rm = TRUE) == 0)
removed_vars_rural <- names(main_WI_rural)[which(zero_sd_vars_rural) + 7]  # +6 to account for starting at column 5
...
main_pca <- psych::principal(
  main_WI[, 8:ncol(main_WI)],        # select only binary variables
...
main_pca_urban <- psych::principal(
  main_WI_urban[, 8:ncol(main_WI_urban)],        # select only binary variables
  rotate = "varimax",           # optional rotation
  nfactors = 1,                 # number of components
  covar = TRUE,
  cor = "tet",
...
main_pca_rural <- psych::principal(
  main_WI_rural[, 8:ncol(main_WI_rural)],        # select only binary variables
  rotate = "varimax",           # optional rotation
  nfactors = 1,                 # number of components
  covar = TRUE,                 # use covariance matrix (not correlation)
  cor = "tet",
...
#using joint calculation & comparing refugees in and out of camp and host community
main_WI %>%
  filter(Intro_07 == "Refugees" | Intro_07 == "Host community", !is.na(wealth_quintile)) %>%
  mutate(
    pop_group = case_when(
      Intro_07 == "Host community"                         ~ "Host community",
      Intro_07 == "Refugees" & Intro_08 == 1              ~ "Refugees (in-camp)",
      Intro_07 == "Refugees" & Intro_08 == 2              ~ "Refugees (out-of-camp)",
      TRUE                                                 ~ NA_character_
    ),
    pop_group = factor(pop_group, levels = c(
      "Host community",
      "Refugees (out-of-camp)",
      "Refugees (in-camp)"
    )),
    wealth_quintile = fct_rev(wealth_quintile)  # Reverse order (if needed)
  ) %>%
  filter(!is.na(pop_group)) %>%
  ggplot(aes(x = pop_group, fill = wealth_quintile)) +
  geom_bar(position = "fill") +
  ggtitle("Wealth Distribution FDS Pakistan") +
...
ggsave("C:/Users/LEOPOLD/OneDrive - UNHCR/Work/DHS Wealth index/figures/quintiles_PAK.png", width = 6, height = 4, dpi = 300)
...
#More Graphs: 
#IF RUNNING URBAN AND RURAL SEPARATELY AND BRINGING TOGETHER USING REGRESSIONS
main_WI %>%
...
main_WI %>%
  filter(Intro_07 == "Refugees" | Intro_07 == "Host community", !is.na(q_combscore)) %>%
  mutate(
    pop_group = case_when(
      Intro_07 == "Host community"                         ~ "Host community",
      Intro_07 == "Refugees" & Intro_08 == 1              ~ "Refugees (in-camp)",
    ...
    q_combscore = fct_rev(q_combscore)  # Reverse order (if needed)
  ) %>%
  filter(!is.na(pop_group)) %>%
  ggplot(aes(x = pop_group, fill = q_combscore)) +
  geom_bar(position = "fill") +
  xlab("Population Group") +
  ylab("Percentage") +
  ggtitle("Wealth Distribution by Population Group") +
  scale_y_continuous(labels = scales::percent_format()) +
  scale_fill_unhcr_d(guide = guide_legend(reverse = TRUE)) +
  theme_unhcr() +
  coord_flip()
